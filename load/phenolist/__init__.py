...
def filter_phenolist(phenolist, filter_func, name_for_debugging=''):
    ...
    if failing_phenos:
        print("Here's the first phenotype that failed:", json.dumps(failing_phenos[0],ensure_ascii=False))
    ...
    for pheno in phenolist:
        for k in pheno:
            if isinstance(pheno[k], (int, float)):
                ...
                pheno[k] = 'json:' + json.dumps(pheno[k],ensure_ascii=False)
        w.writerow(pheno)
...
def write_phenolist_to_file(phenolist, f):
    ...
    json.dump(phenolist, f, sort_keys=True, indent=1,ensure_ascii=False,)
